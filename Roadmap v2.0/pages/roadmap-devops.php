...
<body>
  ...
  <!-- Roadmap -->
  <div id="roadmap" class="container-fluid p-0 align-items-center justify-content-center d-flex h-100">
    <div class="row col-md-12 col-12 p-0">
      ...
      <?php
        ...
        while ($row = mysqli_fetch_array($categoriesSelect)) {
          if ($row != 0) {
            ...
            echo '<div class="col-xxl-2 col-lg-3 col-sm-6 col-10 m-0 p-1 d-flex flex-column border shadow rounded border-primary-subtle">
                    <div class="align-items-center justify-content-center d-flex">
                      <b class="text-center mb-2 border-primary-subtle">'. $row['order_number'] . '. ' . $row['name'] . '</b>
            ...
            while ($row1 = mysqli_fetch_array($topicsSelect)) {
              echo '<li style="font-size:13px;cursor: pointer;" data-topic-id="' . $row1['id'] . '" data-bs-toggle="modal" data-bs-target="#exampleModal" class="mb-1 mx-1 border rounded border-primary-subtle">';
              $topicCount++;
              if ($row1['status'] == 'Done') {
                echo '<span style="font-size:12px;" class="ms-1">🟢  </span>' . ($topicCount < 10 ? '0' : '') . $topicCount . '. ';
              } else {
                echo '<span style="font-size:12px;" class="ms-1">🔘  </span>' . ($topicCount < 10 ? '0' : '') . $topicCount . '. ';
              } 
              echo $row1['name'] .'</li>';
      ...
      ?>
      <div class="col-md-1 col-1 col-empty"></div>
    </div>
  </div>
  <!-- Modal -->
